    ...
    def forward(self, z, noise_scale=0, u=None,v=None,sim_v=False):
        ...
        if noise_scale > 0:
            if self.params["scalar_noise_z"] == "Cov":
                ...
                z +=noise_scale * self.normal.sample(z.shape) *self.std_embed_z(self.R_z).unsqueeze(0).unsqueeze(2).unsqueeze(3)
        return z,v
        ...
        with torch.no_grad():
            ...
            if u is not None:
                ...
                for t in range(time_steps + cut_off):
                    ...
                    z,v = self.forward(
                        z, noise_scale=noise_scale, u=u[:, :, t].unsqueeze(2),v=v,sim_v=sim_v)
                    Z.append(z[:, :, 0])
                    V.append(v[:, :, 0])
                V = torch.stack(V)
                V = V[cut_off:]
...
class Transition_FullRank(nn.Module):
    ...
    paramterised by a full rank network
    """
    ...
    def __init__(
        ...
        if nonlinearity == "clipped_relu":
            self.h = nn.Parameter(
                uniform_init1d(dz), requires_grad=train_neuron_bias
            )
        else:
            self.h = nn.Parameter(
                torch.zeros(dz), requires_grad=train_neuron_bias
            )
        ...
        if self.du > 0:
            self.Wu = nn.Parameter(
                uniform_init2d(dz, self.du), requires_grad=True
            )
        else:
    ...
    def forward(self, z, v=None):
        ...
        z = (
            A * z
            + (1-A)*(torch.einsum("zN,BNTK->BzTK",self.W, self.nonlinearity(z, self.h.unsqueeze(0).unsqueeze(2).unsqueeze(3))))
            + self.hz.unsqueeze(0).unsqueeze(2).unsqueeze(3)
        )
        if v is not None:
