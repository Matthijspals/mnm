...
def train_VAE(
    ...
    for i in range(curr_epoch, training_params["n_epochs"]):
        with torch.no_grad():
            # DO EVALUATION
            if i > 0 and i % training_params["eval_epochs"] == 0 and training_params["run_eval"]:
                vae.eval()
                with torch.no_grad(): 
                    if training_params["neuromodulators"]: 
                        X_pred = predict_X(
                            vae,
                            eval_task.task_params["dur"],
                            eval_task.data[0].T,
                            eval_task.task_input[0].T.unsqueeze(0),
                        ...
                        klx_bin, psH, mean_rate_error = eval_VAE(
                            ...
                            neuromodulators=training_params["neuromodulators"]
                        )
