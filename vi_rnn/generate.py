...
def get_initial_state(
    vae, u, s, x=None, initial_state="prior_sample", optimal_proposal=False, k=1
):
    """
    ...
    with torch.no_grad():
        ...
        if initial_state == "prior_sample":
            ...
            _, Emean, log_Evar, _ = vae.encoder(x_enc)
            #print(Emean.shape)
            #print(log_Evar.shape)
            Evar = torch.clamp(
...
def generate(
    ...
    Args:
        vae (VAE): trained VAE model
        x (torch.tensor; batch_size x dim_x x dim_T): data
        s (torch.tensor; batch_size x dim_T): neuromodulatory signals
        u (torch.tensor; batch_size x dim_u x dim_T): inputs
